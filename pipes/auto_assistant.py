...
class Pipe:
    ...
    _HERMES_BRIEF = (
       ...
       "5b. OUTPUT PROTOCOL — every job's prompt MUST end by instructing: finish your response with these lines, exactly this shape:\n"
        ...
        "6b. NEVER create a job with --no-agent or --script (a standalone script with no model at "
       "run time). Those runs have no reasoning to recover when a page's markup shifts, and a bug "
       "in the generated script fails silently — one such job computed its ALERT text into a "
       "variable it never printed, so the alert could never fire. Always use the normal agent "
       "mode, where each run reasons and emits the LOG/ALERT lines itself.\n"
