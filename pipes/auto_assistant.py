...
class Pipe:
    ...
    _HERMES_BRIEF = (
        ...
        "searching), what to extract, and what counts as noteworthy. Prefer https:// URLs — the "
        ...
        "5. Set every job's delivery to LOCAL (deliver='local'). Origin delivery cannot reach "
        "this chat — the API server is request/response and cannot receive pushes; relying on it "
        "makes every run end in a delivery error. The user-visible result is ONLY the webhook "
        "POST: the job must ALWAYS post its result to the OpenWebUI background-tasks channel by "
        "running: curl -s -X POST \"$(cat ~/.hermes/owui_webhook_url)\" -H 'Content-Type: "
        "application/json' -d '{\"content\": \"<short summary>\"}' — one concise message per run, "
        "leading with the key number/fact and flagging changes prominently (localhost http is "
        "fine there).\n"
