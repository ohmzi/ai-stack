#!/usr/bin/env python3
"""Add a "Continue without an account" link to the sign-in page.

Sends visitors to the public, no-login OhmzAI instance (docs/PUBLIC_INSTANCE.md). The URL is not
hardcoded here: branding/loader.js sets `window.__ohmzGuestUrl` before this component mounts, the
same way it rewrites the app name — see that file for why (a stock, unbranded build then renders
nothing, and the public instance itself never gets a value, so it can't link to itself).

Deliberately a fourth vendored file rather than DOM injection into the running page, matching how
01-03 handle the composer: the injection alternative has no stable selector once Svelte reshuffles
markup, and fails silently — a link that looks right and 404s. This page happens to ship stable ids
(#auth-page, #auth-container, #auth-login-card) that the composer never had, but the patch approach
is kept for the same reason 01-03 use it: `git apply` fails loudly on a moved anchor, in the build,
instead of quietly in someone's browser.
