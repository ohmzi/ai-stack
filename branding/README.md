...
python3 tests/test_branding.py     # 54 checks; --restart adds the restart case (108)
```
...
## The Android install path

Added 2026-09-17, chasing an installed Android app that showed its icon and
never loaded — while the same site was fine in Chrome on the same phone, and
fine installed on iOS. The caching problem above explains the *stale shell*; this
section is a second, independent way Android fails and iOS cannot.

Two files live at the site root, because Android reads both during an install
and **iOS reads neither** — no equivalent step exists on the iOS side, which is
the whole reason the asymmetry is possible:

| Path | Why |
|---|---|
| `/robots.txt` | the verifier reads it **first**, and will not fetch a disallowed path |
| `/.well-known/assetlinks.json` | the Digital Asset Links document: proof the site owns the app |

Both were wrong, in different ways:

- Stock `robots.txt` is `User-agent: *` / `Disallow: /`, so the verifier was
  turned away before it asked for anything. The branded copy keeps the instance
  disallowed — it is private — and adds `Allow: /.well-known/`. Allow and
  Disallow are evaluated together regardless of order and the more specific rule
  wins.
- `/.well-known/assetlinks.json` **did not exist**, and `SPAStaticFiles` answers
  any non-`.js` miss with `index.html` and a **200**. So it returned 11 KB of
  HTML with a `content-type` of `text/html` — a soft 404 claiming `200 OK` in
  the one place that must not lie.

`assetlinks.json` is an **empty array**, deliberately. The package name and
signing fingerprint of a Chrome-minted WebAPK are generated by Google at mint
time and cannot be known from here, so the only truthful thing to serve is "no
app is associated" — valid JSON, real 200, and no pretence. Serving HTML there
was worse than serving nothing, because it looked like an answer.

**Neither file can be fingerprinted, and that matters.** The verifier fetches
exact paths, so there is no `?v=` to move, and Cloudflare caches `/robots.txt`
for four hours like everything else at the root. Changing either one therefore
needs an **edge purge**; until then the origin is right and the edge is stale,
which reads as "the fix didn't work". Confirmed the hard way on the day these
were added: `cf-cache-status: HIT` on `/robots.txt` was still serving the stock
`Disallow: /` after the branded file was installed. Unlike `index.html`, there is
no fingerprint trick available here — purge, or wait out the TTL.
...
| `site-root/` | Files serving at the **site root**, laid out to mirror where they land: `robots.txt` and `.well-known/assetlinks.json`. Android's install checks read both. **Not generated, and not fingerprinted** — see "The Android install path". |
