...
- 2026-08-02 — **Image model swapped: Krea 2 Turbo → RedCraft** (`krea2/redcraft23INT8INT4FP8_30Krea2.safetensors`, a Krea 2-base checkpoint) in both `image_krea.py` and `auto_assistant._build_t2i_wf`. Same CLIP/VAE and the same 8-step / cfg 1.0 / `er_sde`-simple settings — confirmed against the creator's own spec (`ER_SDE/Euler | Simple | CFG=1 | 8-12 Steps`). Krea 2 Turbo left on disk for rollback; status lines now read `RedCraft`. Verified with a live 1024×1024 render (21 s).
- 2026-08-02 — **Conversation-continuity failure found and fixed (all three image pipes).** Reported symptom: "make this picture realistic" after a cat image returned a photo of a father and son; "make this picture animated" in Photoreal returned an unrelated person. Five independent defects, each confirmed from `webui.db` + `media_metrics.jsonl`: (1) OpenWebUI background tasks (`### Task:` title/tag/follow-up/query prompts) were reaching the pipes and running 14–174 s GPU renders — `get_task_model_id` falls back to the *chat's* model when the configured task model is not visible in the registry, and `gemma3:1b` is hidden; (2) those junk renders overwrote each chat's last-image cache, so follow-up edits ran on the wrong image; (3) generated images were invisible to the next turn (OWUI 0.10 stores pipe replies in `message.output` with `content=''`, and the history scans only read str content); (4) `photoreal.py` had no follow-up edit path at all — any text-only follow-up became a fresh SDXL t2i of the follow-up words; (5) vision-QA verified the *rewritten instruction* against only the produced image, so a derailed edit passed with `qa_rounds=0`. Fixes live in a new shared sidecar `pipes/shared/media_session.py`: `__task__`/`### Task:` guard answering task prompts as text on `gemma3:1b`, a persistent per-chat last-image store shared by all three pipes (survives deploys and mid-chat model switches), reference recovery across every OWUI 0.10 message shape, subject-agnostic style-conversion instructions, and two new prompt contracts ported from the official Qwen-Image *Edit Prompt Enhancer* (the rewriter now SEES the reference image) and the ImgEdit judge (edit QA scores adherence **and** preservation on both images against the user's original wording). Prompt-enhance rebuilt per RedCraft/Krea-2 guidance — photographic register by default, no in-prompt style examples to copy (the old one's cartoon example is why an unstyled request came out as a cartoon). Routing guards fixed across the pipes ("make this picture X" is never a fresh render; trailing-`?` and "have them …" follow-ups edit instead of falling to chat). Covered by `tests/test_continuation.py` (49 checks, no GPU); all existing suites re-run green; deployed (Assistant 0.6.0, Image 1.4.0, Photoreal 0.6.0) and verified end-to-end on GPU with a fresh pipe instance and an OWUI-shaped history. Full writeup: `docs/IMAGE_CONTINUATION.md`.
